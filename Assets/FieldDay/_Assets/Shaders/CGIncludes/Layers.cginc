...
{
    return half4(color.rgb, color.a * SampleSingle(alphaTexture, uv));
}
...
inline half4 LayerIntensityTexture(sampler2D intensityTexture, half2 uv, half4 color)
{
    float intensity = SampleSingle(intensityTexture, uv);
...
inline half4 LayerIntensityAlphaTexture(sampler2D intensityTexture, half2 uv, half4 color)
{
    float intensity = SampleSingle(intensityTexture, uv);
    return half4(color.rgb * intensity, color.a * step(0.001, intensity));
}
